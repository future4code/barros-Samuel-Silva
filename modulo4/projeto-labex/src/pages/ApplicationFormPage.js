...
import { StyleFormExt, StyleButton} from "../style";
...
import iconOk from "../img/icon-ok.png"
    ...
    const [form, onChange, clear] = useForm({ trip:"", name: "", age: "", applicationText: "", profession: "", country: "", })
    const [idTrip, setIdTrip] = useState();
    const [dataListTrips, isLoadingListTrips, erroListTrips] = useRequestData(`${BASE_URL}`)   
    const lista = dataListTrips && dataListTrips.map((item, index) => {
      return (
    ...
    const applyTrip = (event) => {
        ...
        axios.post(`${BASE_URL}${idTrip}/apply`, body,
    ...
    return (
        <StyleFormExt onSubmit={applyTrip}>
            ...
            <label htmlFor="tripSelect">Escolha sua viagem: </label>
            <select
                id="tripSelect"
                name="tripSelect"
                type="text"
                required
                onChange={(e) => setIdTrip(e.target.value)}
                value={idTrip}>
